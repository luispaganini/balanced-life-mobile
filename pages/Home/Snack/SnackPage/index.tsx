...
import { getMealsAsync } from "@/services/snack/snack";
...
export default function SnackPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const snack = await getMealsAsync(snackStore.date);
      snackStore.setData(snack);
      setPieChartData(
        getDataPieChart(
          [
            {
              nameKey: t("Carbohydrates"),
              population: snack.carbohydrates || 1,
              color: Colors.color.blue,
            },
            {
              nameKey: t("Protein"),
              population: snack.protein || 1,
              color: Colors.color.red,
            },
            {
              nameKey: t("Fat"),
              population: snack.fat || 1,
          ...
          {snackStore.data ? (
            <SnackContainer>
              ...
              <TitleText type="title">
                {snackStore.data.totalCalories} Kcal
              </TitleText>
              <DatePickerComponent
                onChange={snackStore.setDate}
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <CardSnack
                    idMeal={item.id}
                    idTypeSnack={item.typeSnack.id ? item.typeSnack.id : 0}
                    title={item.typeSnack.name}
                    description={item.totalCalories.toString()}
                  />
                )}
                ListEmptyComponent={() => (
